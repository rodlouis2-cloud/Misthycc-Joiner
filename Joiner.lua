...
NotifSound.Name = "MisthyccNotifSound"
...
local function performJoinSpam(jobId)
    ...
    task.spawn(function()
        ...
        local i = 1
        while currentlyJoining and _G.MisthyccRunning do
    ...
    task.spawn(function()
        ...
        for i = 1, attempts do
            if not _G.MisthyccRunning then break end
            pcall(function()
...
-- CHROMA LOOP
-- ═══════════════════════════════════
_G.MisthyccRunning = true

task.spawn(function()
    while _G.MisthyccRunning do
        local tk = tick()
...
task.spawn(function()
    while _G.MisthyccRunning do
        pcall(function()
            ...
            local headers = {["Cache-Control"]="no-cache",["User-Agent"]="Roblox/MisthyccUI"}
            local response
